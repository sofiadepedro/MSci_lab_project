...
if __name__ == "__main__":
    ...
    llaves = median_data_blind.keys()

    filepath = os.path.join(path, 'median_data_blind.csv')
    ...
    for foldername in os.listdir(path):
        if foldername.startswith("test_"):
            ...
            if os.path.isfile(f'{folder_path}/{scaling_data}.csv'):
                median_data_blind["subject"] = subject
                ...
                for ind, cond in enumerate(scaling_touch_conds.keys()):
                    ...
                    for delta in deltas:
                        ...
                        print("response", scaling_touch_conds[cond][f"scaling_delta{delta}_response_list"])
                ...
                figure_name = f"median_blindcheck_test_{todaydate}"
                ...
                for cond in scaling_touch_conds.keys():
                    plt.scatter(deltas, scaling_touch_conds[cond]["scaling_medians"], c = scaling_touch_conds[cond]["color"], marker="o", s=s_mean)
                    plot_linregression(
                        deltas, 
                        scaling_touch_conds[cond]["scaling_medians"], 
                        scaling_touch_conds[cond]["color"], 
                ...
                for cond in scaling_touch_conds.keys():
                    ...
                    median_data_blind[f"slope_{cond}"], median_data_blind[f"intercept_{cond}"] = get_linregression(deltas, scaling_touch_conds[cond]["scaling_medians"], cond)

                    all_response_list = list(
                        scaling_touch_conds[cond]["table"]["response"]
                    )

                    median_data_blind[f"min_value_{cond}"] = min(all_response_list)
                    median_data_blind[f"max_value_{cond}"] = max(all_response_list)

        
                ## GET PERCEIVED INTENSITY OF THRESHOLD VALUE
                if os.path.isfile(f'{folder_path}/{staircase_data}.csv'):

                    median_data_blind["threshold"] = getThreshold(test_state="test", subj=todaydate)

                    for cond in scaling_touch_conds.keys():
                        median_data_blind[f"perceived_threshold_intensity_{cond}"] = np.interp(median_data_blind["threshold"], deltas, scaling_touch_conds[cond]["scaling_medians"])
                else:
                    median_data_blind["threshold"] = np.NaN
                    for cond in scaling_touch_conds.keys():
                        median_data_blind[f"perceived_threshold_intensity_{cond}"] = np.NaN
                ...
                if os.path.isfile(f'{folder_path}/{anchoring_data}.csv'):
                    ...
                    median_data_blind[f"anch_zero_percentage_correct"] = get_percentage_correct(delta_zero_list, "0")
                    median_data_blind[f"anch_hundred_percentage_correct"] = get_percentage_correct(delta_trescinco_list, "100")
                    
    

                ## SAVING DATA
                save_all_data(filepath, median_data_blind, subject)

                median_data_blind.clear()