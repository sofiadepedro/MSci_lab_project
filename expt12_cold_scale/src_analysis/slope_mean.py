...
if __name__ == "__main__":
    ...
    filepath = os.path.join(path, 'slope_mean_blind.csv')
        ...
        if foldername.startswith("test_"):
            ...
            if os.path.isfile(f'{folder_path}/{scaling_data}.csv') and os.path.isfile(f'{folder_path}/{staircase_data}.csv'):
                ...
                for cond in scaling_touch_conds.keys():
                    ...
                    (slope_data_blind[f"slope_{cond}"], slope_data_blind[f"intercept_{cond}"], slope_data_blind[f"rvalue_{cond}"], slope_data_blind[f"pvalue_{cond}"], slope_data_blind[f"stderrt_{cond}"]) = stats.linregress(deltas, scaling_touch_conds[cond]["scaling_means"])
