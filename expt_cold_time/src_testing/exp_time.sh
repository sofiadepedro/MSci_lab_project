...
clear
name_scripts=('Thermal camera check' 'Zabers check' 'Touch stability' 'Height finding' 'Extrapolation of heights' 'Grid ROI finding' 'Training Staircase' 'Staircase' 'Delta calculation' 'SDT' 'Send data to computer')
...
if (( $index_start < 9 )); then
    ...
    python ${path}/delta_calc.py $1 $2 $3 $4 "-ns" "1"
...
if (( $index_start < 10 )); then
    ...
    python ${path}/exp_sdt.py $1 $2 $3 $4
